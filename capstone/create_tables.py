...
def create_state_dimension(input_df, output_data):
    ...
    df = input_df.select(
        ["State Code", "State", "Median Age", "Male Population", "Female Population", "Total Population",
...
def create_airport_dimension(input_df, output_data):
    ...
    df = input_df.select(
        ["ident", "type", "iata_code", "name", "iso_country", "iso_region", "municipality", "gps_code", "coordinates",
         "elevation_ft"]) \
...
def create_immigration_fact(immigration_spark, output_data, spark):
    ...
    df = immigration_spark.select(["*"]) \
        ...
        .join(migrant, (immigration_spark.biryear == migrant.birth_year) & (immigration_spark.gender == migrant.gender),
              how='full') \
        .join(status, (immigration_spark.entdepa == status.arrival_flag) & (
                immigration_spark.entdepd == status.departure_flag) & \
        ...
        .select(["cicid", "i94res", "depdate", "i94mode", "i94port", "i94cit", "i94addr", "airline", "fltno", "ident",
                 "country_code", \
                 "temperature_id", "migrant_id", "status_flag_id", "visa_id", "state_code",
                 time.arrdate.alias("arrdate")])
